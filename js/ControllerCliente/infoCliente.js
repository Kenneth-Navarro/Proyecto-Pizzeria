...
function LlenaTabla(TextoJSON) {
  ...
  $("#tablaClientes thead").append("<tr>");
  $("#tablaClientes thead tr").append("<th scope='col'>Cédula </th>");
  $("#tablaClientes thead tr").append("<th scope='col'>Nombre </th>");
  $("#tablaClientes thead tr").append("<th scope='col'>Primer Apellido </th>");
  $("#tablaClientes thead tr").append("<th scope='col'>Segundo Apellido </th>");
  $("#tablaClientes thead tr").append("<th scope='col'></th>");
  $("#tablaClientes thead tr").append("<th scope='col'></th>");
  $("#tablaClientes thead tr").append("<th scope='col'></th>");

  // Agrega el cuerpo de la tabla
  $("#tablaClientes").append("<tbody class='table-group-divider'>");
  ...
  for (let i = 0; i < ObjetoJson.length; i++) {
    ...
    $("#tablaClientes tbody tr:last-child").append(
      "<th scope='row'>" + ObjetoJson[i].Cedula + " </th>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td scope='row'>" + ObjetoJson[i].Nombre + " </td>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td scope='row'>" + ObjetoJson[i].PrimerApellido + " </td>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td scope='row'>" + ObjetoJson[i].SegundoApellido + " </td>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td>" +
        "<a class=' botonosTabla btn btn-outline-primary' href='actualizarCliente.php?id=" +
        ObjetoJson[i].Cedula +
        "'>Actualizar <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-arrow-repeat' viewBox='0 0 16 16'><path d='M11.534 7h3.932a.25.25 0 0 1 .192.41l-1.966 2.36a.25.25 0 0 1-.384 0l-1.966-2.36a.25.25 0 0 1 .192-.41zm-11 2h3.932a.25.25 0 0 0 .192-.41L2.692 6.23a.25.25 0 0 0-.384 0L.342 8.59A.25.25 0 0 0 .534 9z'/><path fill-rule='evenodd' d='M8 3c-1.552 0-2.94.707-3.857 1.818a.5.5 0 1 1-.771-.636A6.002 6.002 0 0 1 13.917 7H12.9A5.002 5.002 0 0 0 8 3zM3.1 9a5.002 5.002 0 0 0 8.757 2.182.5.5 0 1 1 .771.636A6.002 6.002 0 0 1 2.083 9H3.1z'/></svg>        </a></td>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td>" +"<a class=' botonosTabla btn btn-outline-success' href='verCliente.php?id=" + ObjetoJson[i].Cedula + "'>Ver <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-folder2-open' viewBox='0 0 16 16'><path d='M1 3.5A1.5 1.5 0 0 1 2.5 2h2.764c.958 0 1.76.56 2.311 1.184C7.985 3.648 8.48 4 9 4h4.5A1.5 1.5 0 0 1 15 5.5v.64c.57.265.94.876.856 1.546l-.64 5.124A2.5 2.5 0 0 1 12.733 15H3.266a2.5 2.5 0 0 1-2.481-2.19l-.64-5.124A1.5 1.5 0 0 1 1 6.14V3.5zM2 6h12v-.5a.5.5 0 0 0-.5-.5H9c-.964 0-1.71-.629-2.174-1.154C6.374 3.334 5.82 3 5.264 3H2.5a.5.5 0 0 0-.5.5V6zm-.367 1a.5.5 0 0 0-.496.562l.64 5.124A1.5 1.5 0 0 0 3.266 14h9.468a1.5 1.5 0 0 0 1.489-1.314l.64-5.124A.5.5 0 0 0 14.367 7H1.633z'/></svg></a></td>"
    );
    $("#tablaClientes tbody tr:last-child").append(
      "<td>" +
